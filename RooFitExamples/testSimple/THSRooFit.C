...
void THSRooFit::Fit(Bool_t randPar){   
  ...
  if(fBinnedFit){
    ...
    if(fData->isNonPoissonWeighted())fResult=fModel->chi2FitTo(rdh,RooFit::Save(kTRUE),RooFit::Extended(),RooFit::Save(kTRUE),RooFit::DataError(RooAbsData::SumW2));
    else fResult=fModel->chi2FitTo(rdh,RooFit::Save(kTRUE),RooFit::Extended(),RooFit::Save(kTRUE),RooFit::DataError(RooAbsData::SumW2));
...
void THSRooFit::MergeModelSpecies(){
  ...
  yields.remove(*(yields.find(fYld+fSingleSp)));
  yields.setAttribAll("Constant");
  RooAddPdf* rap=new RooAddPdf(fSingleSp+"Merged",fSingleSp+"Merged",pdfs,yields);
  // fWS->import(*rap);

  //remove all pdfs and add back single species and summed pdfs
  fPDFs.releaseOwnership();
  fPDFs.removeAll();
  fPDFs.add(*(fWS->pdf(fSingleSp)));
  fPDFs.add(*rap);

  fYields.releaseOwnership();
  fYields.removeAll();
  fYields.add(*(fWS->var(fYld+fSingleSp))); //add back in single species yld
...
void THSRooFit::FitMany(Int_t Nfits){
  ...
  fWS->saveSnapshot(Form("ssFit%d",0),fParameters,kTRUE);
  ...
  //fWS->loadSnapshot(Form("YssFit%d",best));
